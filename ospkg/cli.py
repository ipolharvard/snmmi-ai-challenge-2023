...
class ProjectCLI(click.MultiCommand):
    ...
    ospkg.commands package. Each module in that
    ...
    def __init__(self, *args: Any, **kwargs: Any):
        ...
        for mod_info in pkgutil.iter_modules(commands.__path__):
            name = mod_info.name
            full_name = f"ospkg.commands.{name}"
            ...
            if hasattr(mod, name):
                ...
                if isinstance(cmd, click.Command):
                    ...
                    raise TypeError(
                        f"The function {name} in module {full_name} is not of "
                        "type click.Command."
                    )
            else:
...
def run_cli() -> None:
    ...
    This is the entrypoint for any project task.
    """
    cli = ProjectCLI(help="Main entrypoint for the os project.")
    cli()
